...
class MainActivity : ComponentActivity() {
        ...
        factoryProducer = {
            object : ViewModelProvider.Factory {
                ...
                override fun <T : ViewModel> create(modelClass: Class<T>, extras: CreationExtras): T {
                    val app = application as GloamApplication
                    return GloamViewModel(app.database.repository) as T
                }
            }
        }
    )
    ...
    private fun exportData(uri: Uri) {
        lifecycleScope.launch {
            runCatching {
                // Collect one snapshot from the Flow
                val entries = mutableListOf<JournalEntry>()
                val job = launch {
                    viewModel.allEntries.collect { entries.addAll(it); throw Exception("done") }
                }
                job.join()
                // job threw to exit the collect — entries now populated
    ...
    private fun writeExportJson(uri: Uri, entries: List<JournalEntry>) {
        ...
        val root = JSONObject().apply {
            put("app", "Gloam")
            put("exported", java.time.LocalDateTime.now().toString())
        ...
        PinLockScreen(
            ...
            onPinEntered = { pin ->
                context.getSharedPreferences("gloam_secure", Context.MODE_PRIVATE)
                    .edit().putString("pin_hash", hashPin(context, pin)).apply()
                showPinSetup = false
                true
            }
        )
        ...
        NavHost(
            ...
            composable(Screen.Settings.route) {
                ...
                SettingsScreen(
                    ...
                    onExportData = {
                        exportLauncher.launch("gloam_export_${LocalDate.now()}.json")
                    },
